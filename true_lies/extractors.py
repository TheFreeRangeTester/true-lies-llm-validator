...
# Extractores genéricos reutilizables
EXTRACTORS = {
    'currency': lambda text: re.findall(r'\$[\d,]+\.?\d*', text)[0] if re.findall(r'\$[\d,]+\.?\d*', text) else None,
    'usd_currency': lambda text: re.findall(r'(?:USD|dolares?)\s*(\d+)', text, re.IGNORECASE)[0] if re.findall(r'(?:USD|dolares?)\s*(\d+)', text, re.IGNORECASE) else None,
